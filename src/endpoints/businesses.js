...
export function generate(destination) {
  ...
  const objects = Array(NB_BUSINESSES)
    .fill()
    .map((_, i) => {
      const id = i + 1;

      const reviews = Array(Math.floor(Math.random() * 10))
        .fill()
        .map((_, j) => ({
      ...
      return {
        ...
        address: {
          ...
          street: faker.fake(
            '{{random.number}} {{address.streetName}} {{address.streetSuffix}}'
          )
        },
        position: { lat: lat, lng: lng },
        grade:
          reviews.reduce((acc, value) => acc + value.grade, 0) / reviews.length,
        reviews
      };
    });
